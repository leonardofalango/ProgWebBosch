...
# Carregando os dados
complete_data = pd.read_csv('S:/COM/Human_Resources/01.Engineering_Tech_School/02.Internal/10 - Aprendizes/6 - Programador Web 2022/Leonardo Falango/Módulo 5/Exercícios/Dados/superheroes_nlp_dataset.csv')
# Selecionado as colunas
complete_data = complete_data[['history_text', 'creator']]
# Convertendo em lista
# Entrada para o nltk
complete_data = list(complete_data.itertuples(index=False,name=None))
# Separando as bases
data_train, data_test = train_test_split(complete_data, test_size=0.10)

################### Train ######################
data_train_str = []
for i in data_train:
    data_train_str.append(str(i))
